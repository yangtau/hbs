...
public class HBaseStorage implements Storage {
    ...
    public CompletableFuture<Void> put(KeyValue.Key key, byte[] value) {
        return put(key.table(), key.row(), key.col(), value);
    }
    ...
    public CompletableFuture<byte[]> get(KeyValue.Key key) {
        return get(key.table(), key.row(), key.col());
    }
    ...
    public CompletableFuture<Boolean> exists(KeyValue.Key key) {
        return exists(key.table(), key.row(), key.col());
    }
    ...
    public CompletableFuture<Void> remove(KeyValue.Key key) {
        return remove(key.table(), key.row(), key.col());
    }

    // Read data and record the newest read timestamp for each version.
    // In HBase, we use @GetEndpoint to implement this.
    public CompletableFuture<KeyValue.Value> getWithReadTimestamp(String table, byte[] row, byte[] col, long readTimestamp) {
        ...
        return GetEndpoint.runAsync(connection, table, row, request)
                .thenApply(
                        response -> {
                            ...
                            return new KeyValue.Value(response.getValue().toByteArray(),
    ...
    public CompletableFuture<KeyValue.Value> getWithReadTimestamp(KeyValue.Key key, long readTimestamp) {
        return getWithReadTimestamp(key.table(), key.row(), key.col(), readTimestamp);
    }
    ...
    public CompletableFuture<Boolean> putIfNoConflict(KeyValue.Key key, byte[] value, long writeTimestamp) {
        return putIfNoConflict(key.table(), key.row(), key.col(), value, writeTimestamp);
    }

    @Override
    public CompletableFuture<Void> cleanCell(KeyValue.Key key, long timestamp) {
        var delete = new Delete(key.row())
                .addFamilyVersion(key.col(), timestamp);
        return connection.getTable(TableName.valueOf(key.table()))
                .delete(delete);
    }

    private CompletableFuture<Void> deletes(Collection<KeyValue.Key> keys,
                                            Function<KeyValue.Key, Delete> createDelete) {
        var futures = keys.stream()
                .collect(Collectors.groupingBy(KeyValue.Key::table, // group by table name
                        Collectors.mapping(createDelete, Collectors.toList()))) // table -> List<Delete>
                .entrySet()
                .stream()
                .map(e -> connection.getTable(TableName.valueOf(e.getKey())).deleteAll(e.getValue()))
    ...
    @Override
    public CompletableFuture<Void> cleanCells(Collection<KeyValue.Key> keys, long timestamp) {
        return deletes(keys,
                k -> new Delete(k.row()).addFamilyVersion(k.col(), timestamp));
    }

    @Override
    public CompletableFuture<Void> cleanUncommittedFlag(Collection<KeyValue.Key> keys, long timestamp) {
        return deletes(keys,
                k -> new Delete(k.row()).addColumn(k.col(), Constants.UncommittedQualifierBytes, timestamp));
    }
}
