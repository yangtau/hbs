...
class GetEndpointTest {
    ...
    private void expectGet(String row, long ts, byte[] expectValue, long expectVersion, boolean expectCommitted)
        ...
        var rsp = GetEndpoint.runAsync(conn, tableName, request).get();
    ...
    private void checkReadTimestamp(String row, long version, long expectRt)
            throws ExecutionException, InterruptedException {
        var res = TestUtil.get(conn, tableName, row, version, family);
    ...
    @Test
    void getCommittedAndUncommitted() throws ExecutionException, InterruptedException {
        TestUtil.DeleteTable(conn, tableName);
        TestUtil.CreateTable(conn, tableName, family);
        var value = "hello";

        // Read uncommitted
        // get with a small ts, no RT should be written
        TestUtil.put(conn, tableName, "row1", 1L, family, Constants.DataQualifier, value);
        TestUtil.put(conn, tableName, "row1", 1L, family, Constants.CommitQualifier, "");
        ...
        // Read committed
        TestUtil.put(conn, tableName, "row1", 3L, family, Constants.DataQualifier, value);
    ...
    @Test
    void getFromMultiVersions() throws ExecutionException, InterruptedException {
        TestUtil.DeleteTable(conn, tableName);
        TestUtil.CreateTable(conn, tableName, family);
        ...
        for (long i = 0L; i < length; i += 3L) {
            ...
            TestUtil.put(conn, tableName, row, ts, family, Constants.DataQualifier, value);
    ...
    @Test
    void multiGetOnTheSameData() throws ExecutionException, InterruptedException {
        TestUtil.DeleteTable(conn, tableName);
        TestUtil.CreateTable(conn, tableName, family);
        ...
        long writeTs20 = 20L;
        TestUtil.put(conn, tableName, row, writeTs20, family, Constants.DataQualifier, value20);

        // version 15
        String value15 = "lisp";
        long writeTs15 = 15L;
        TestUtil.put(conn, tableName, row, writeTs15, family, Constants.DataQualifier, value15);
    ...
    @Test
    void randomGet() throws ExecutionException, InterruptedException {
        TestUtil.DeleteTable(conn, tableName);
        TestUtil.CreateTable(conn, tableName, family);
